...
fun OrderScreen(tableId: Int, navController: androidx.navigation.NavController) {
    ...
    // UI state to force recomposition
...
fun AdminScreen(navController: androidx.navigation.NavController) {
    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        Text("관리자 페이지", style = MaterialTheme.typography.headlineMedium)
        
        // Simplified Admin: Shows total sales
        val totalSales = DataManager.receipts.sumOf { it.totalAmount }
        Text("총 매출: ${totalSales}원", style = MaterialTheme.typography.titleLarge)
        
        LazyColumn(modifier = Modifier.weight(1f).padding(vertical = 16.dp)) {
            items(DataManager.receipts) { receipt ->
                Card(modifier = Modifier.fillMaxWidth().padding(4.dp)) {
                    Column(modifier = Modifier.padding(8.dp)) {
                        Text("날짜: ${receipt.date}")
                        Text("결제금액: ${receipt.totalAmount}원")
                    }
                }
            }
        }
